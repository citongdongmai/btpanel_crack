...
get_node_url(){
	...
	if [ $j = 5000 ];then
	  # NODE_URL='http://download.bt.cn';
		NODE_URL='https://raw.githubusercontent.com/lhpmain/btpanel_crack/master/ip6bt';
	else
		NODE_URL=${urls[$j]}
	fi
...
Install_RPM_Pack(){
	...
	if [ "${yumBaseUrl}" ];then 
		checkYumRepo=$(curl --connect-timeout 5 --head -s -o /dev/null -w "%{http_code} %{time_total}" ${yumBaseUrl})	
		yumStatus=$(echo ${checkYumRepo}|awk '{print $1}')
		time_total=$(echo ${checkYumRepo}|awk '{print $2}'|cut -d '.' -f 1)
	fi
	if [ "${yumStatus}" != "200" ] || [ "${time_total}" -ge "2" ];then
		wget -O yumRepo_select.sh http://download.bt.cn/install/5/yumRepo_select.sh
